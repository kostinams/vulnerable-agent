...
class CognitiveServicesCredential:
    ...
    def get_token(self, *scopes, **kwargs):
        ...
        return self._credential.get_token("https://cognitiveservices.azure.com/.default", **kwargs)
...
def _model_name() -> str:
    return os.getenv("AZURE_INFERENCE_MODEL", "gpt-4o")
...
def _create_llm(agent_name: str, *, temperature: float, session_id: str) -> AzureAIChatCompletionsModel:
    ...
    if not endpoint:
        raise RuntimeError("AZURE_INFERENCE_ENDPOINT is not set")
    
    # Use custom credential that requests correct Cognitive Services scope
    credential = CognitiveServicesCredential()
    
    return AzureAIChatCompletionsModel(
        endpoint=endpoint,
        credential=credential,
        model=_model_name(),
        # temperature parameter not supported by gpt-5-mini
        tags=tags,
        metadata=metadata,
    )
